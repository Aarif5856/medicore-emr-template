...
function emergencyContact(patient: Patient): { name: string; phone: string } {
  ...
  const last = useFamilyName ? patient.lastName : EMERGENCY_FIRST[(seed >> 4) % EMERGENCY_FIRST.length];
...
export function getVitals(patient: Patient): Vital[] {
  ...
  const hr = clamp(
    64 + ((seed >> 4) % 18) + (critical ? 22 : treatment ? 8 : 0),
    52,
    118,
  );
...
export function getMedications(patient: Patient): Medication[] {
  const seed = hashId(patient.id);
  const count =
    patient.status === "Critical" ? 4 : patient.status === "Under Treatment" ? 3 : 2;
...
export function getDocuments(patient: Patient): DocFile[] {
  return [
    { id: `D-${patient.id}-1`, name: "Insurance_Card.pdf", size: "412 KB", uploadedAt: dateOffsetFromLastVisit(patient.lastVisit, -320), kind: "pdf" },
    { id: `D-${patient.id}-2`, name: `ECG_${patient.lastVisit}.pdf`, size: "1.2 MB", uploadedAt: patient.lastVisit, kind: "pdf" },
    { id: `D-${patient.id}-3`, name: "Chest_Xray.jpg", size: "3.8 MB", uploadedAt: dateOffsetFromLastVisit(patient.lastVisit, -180), kind: "image" },
    { id: `D-${patient.id}-4`, name: "Discharge_Summary.docx", size: "88 KB", uploadedAt: dateOffsetFromLastVisit(patient.lastVisit, -420), kind: "doc" },
  ];
}
