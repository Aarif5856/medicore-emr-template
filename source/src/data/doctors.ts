...
const REVIEW_SNIPPETS: Array<Omit<DoctorReview, "id">> = [
  {
    ...
    rating: 5,
    date: "2026-06-14",
    comment: "Took time to explain every step. Never felt rushed.",
  },
  {
    patient: "R. Ito",
    rating: 5,
    date: "2026-06-02",
    comment: "Clear, warm, and very thorough. Highly recommend.",
  },
  {
    patient: "J. Bello",
    rating: 4,
    date: "2026-05-19",
    comment: "Wait was a bit long but the consultation was excellent.",
  },
  {
    patient: "L. Anders",
    rating: 5,
    date: "2026-05-08",
    comment: "Best specialist I've seen at this clinic.",
  },
];
...
export function nextDoctorId(existing: Doctor[]): string {
  const nums = existing.map((d) => Number(d.id.replace("DR-", ""))).filter((n) => !Number.isNaN(n));
