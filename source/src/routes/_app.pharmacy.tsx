...
function PharmacyPage() {
  ...
  const handleCancelRx = (rx: Prescription) => {
    ...
    setViewRx((prev) => (prev && prev.id === rx.id ? { ...prev, status: "Cancelled" } : prev));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as "inventory" | "prescriptions")}>
      ...
      <div
        className={cn(
          "grid gap-3",
          stats.length === 4 ? "grid-cols-2 md:grid-cols-4" : "grid-cols-1 sm:grid-cols-3",
        )}
      >
        {stats.map((s) => (
