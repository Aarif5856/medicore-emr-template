...
function LaboratoryPage() {
  ...
  const handleMarkCritical = (t: LabTest) => {
    ...
    toast.success(
      next === "Critical" ? `Marked ${t.id} as critical` : `Cleared critical flag on ${t.id}`,
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button size="sm" className="gap-2 glow-primary" onClick={() => setNewOrderOpen(true)}>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        {stats.map((s) => (
          <MiniStatCard key={s.label} label={s.label} value={s.value} tone={s.tone} />
        ))}
      </div>
      ...
      <NewOrderDialog open={newOrderOpen} onOpenChange={setNewOrderOpen} onCreate={addTest} />
