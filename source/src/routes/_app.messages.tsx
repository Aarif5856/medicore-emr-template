...
function MessagesPage() {
  ...
  const handleMessageStaff = (s: StaffMember) => {
    ...
    const conv = conversations.find((c) => c.kind === "staff" && c.refId === s.id);
    if (conv) {
      setActiveId(conv.id);
      setMobileView("thread");
  ...
  useEffect(() => {
    if (!staffId) return;
    const conv = conversations.find((c) => c.kind === "staff" && c.refId === staffId);
    if (conv) {
      setActiveId(conv.id);
      setMobileView("thread");
      setConversations((prev) => prev.map((c) => (c.id === conv.id ? { ...c, unread: 0 } : c)));
    ...
    <div className="flex h-full min-h-0 flex-col space-y-4">
      ...
      <Card className="card-glass min-h-0 flex-1 overflow-hidden p-0">
        <div className="grid h-[calc(100vh-14rem)] min-h-[540px] grid-cols-1 md:grid-cols-[340px_minmax(0,1fr)]">
          ...
          <section className={cn("min-h-0", mobileView === "list" ? "hidden md:block" : "block")}>
            {active ? (
              <ConversationThread
                conversation={active}
                onSend={handleSend}
