        ...
        <CardContent className="p-5">
          <div className="grid grid-cols-1 gap-5 md:grid-cols-[auto_minmax(0,1fr)_auto] md:items-center">
            ...
            <div className="min-w-0 space-y-2">
              ...
              <div className="text-xs text-muted-foreground tabular">Patient ID · {patient.id}</div>
            ...
            <div className="flex flex-wrap gap-2 md:flex-nowrap md:justify-end">
              ...
              <Button size="sm" className="gap-2 glow-primary" onClick={() => setBookingOpen(true)}>
          ...
          {labs.map((r) => (
            <li key={r.id} className="flex items-center gap-3 rounded-md px-3 py-3">
              ...
              <Button
                ...
                size="icon"
                className="h-8 w-8"
                aria-label={`Download ${r.test} PDF`}
              >
      ...
      <div className="overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {invoices.map((i) => (
              <TableRow key={i.id}>
                <TableCell className="whitespace-nowrap text-xs font-medium text-foreground tabular">
                  {i.id}
                </TableCell>
                <TableCell className="whitespace-nowrap text-sm text-muted-foreground tabular">
                  {formatDate(i.date)}
                ...
                <TableCell className="whitespace-nowrap text-end text-sm font-semibold text-foreground tabular">
                  {i.amount}
                </TableCell>
                <TableCell className="text-end">
                  <span
                    className={cn(
                      "rounded-full px-2 py-0.5 text-[11px] font-semibold",
                      INVOICE_STATUS_STYLES[i.status],
                    )}
                  >
                    {i.status}
                  </span>
      ...
      {documents.map((d) => {
        ...
        return (
          <Card key={d.id} className="card-glass p-4 transition-colors hover:bg-muted/40">
            <div
              className={cn("mb-3 grid h-10 w-10 place-items-center rounded-lg", DOC_TONES[d.kind])}
            >
