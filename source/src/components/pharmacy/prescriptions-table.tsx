...
export function PrescriptionsTable({ prescriptions, onView, onDispense, onCancel }: Props) {
  ...
  const columns = useMemo<ColumnDef<Prescription>[]>(
    () => [
      ...
      },
      {
        accessorKey: "status",
        header: "Status",
        cell: ({ getValue }) => <PrescriptionStatusBadge status={getValue<PrescriptionStatus>()} />,
      },
      {
        id: "actions",
        cell: ({ row }) => {
            ...
            <div className="flex justify-end" onClick={(e) => e.stopPropagation()}>
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-44">
                  ...
                  <DropdownMenuItem onClick={() => onDispense(rx)} disabled={!canDispense}>
  ...
  const hasActiveFilters = Boolean(search) || status !== "All" || dateRange !== "All time";
  const clearFilters = () => {
    setSearch("");
    setStatus("All");
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select value={status} onValueChange={(v) => setStatus(v as PrescriptionStatus | "All")}>
