...
const schema = z.object({
  ...
  category: z.enum(["Hematology", "Biochemistry", "Radiology", "Microbiology", "Pathology"]),
...
export function NewOrderDialog({ open, onOpenChange, onCreate }: Props) {
  ...
  const onSubmit = (values: FormValues) => {
    ...
    toast.success(`Lab order created for ${patient.firstName} ${patient.lastName} - ${def.name}`);
    onOpenChange(false);
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Order a new lab test. Reference panels are attached automatically once results are
            entered.
          </DialogDescription>
        </DialogHeader>
...
function PatientCombobox({ value, onChange }: { value: string; onChange: (id: string) => void }) {
...
function TestCombobox({ value, onChange }: { value: string; onChange: (name: string) => void }) {
  const [open, setOpen] = useState(false);
            ...
            {grouped.map(([cat, list]) => (
              <CommandGroup key={cat} heading={cat}>
                {list.map((t) => (
                  ...
                  >
                    <Check
                      className={cn("me-2 h-4 w-4", value === t.name ? "opacity-100" : "opacity-0")}
                    />
