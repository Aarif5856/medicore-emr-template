...
export function PatientsTable({ exportRef }: PatientsTableProps = {}) {
  ...
  const columns = useMemo<ColumnDef<Patient>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ getValue }) => <span className="text-sm tabular">{getValue<number>()}</span>,
      },
      {
  ...
  return (
    <div className="space-y-3">
      ...
      {isLoading ? (
        <TableSkeleton
          columns={["w-6", "w-40", "w-16", "w-10", "w-14", "w-32", "w-24", "w-32", "w-20", "w-8"]}
        />
      ) : isError ? (
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="text-xs text-muted-foreground tabular">
          ...
          {Object.keys(rowSelection).length > 0 && (
            <span className="ms-3">· {Object.keys(rowSelection).length} selected</span>
          )}
        </div>
...
function PageNumbers<TData>({ table }: { table: import("@tanstack/react-table").Table<TData> }) {
