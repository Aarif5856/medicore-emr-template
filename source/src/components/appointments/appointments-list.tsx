...
export function AppointmentsListTable({ appointments, onView, onReschedule, onCancel }: Props) {
  ...
  const columns = useMemo<ColumnDef<Appointment>[]>(
    () => [
      ...
      },
      {
        accessorKey: "status",
        header: "Status",
        cell: ({ getValue }) => <AppointmentStatusBadge status={getValue<AppointmentStatus>()} />,
        filterFn: (row, _id, value: string) =>
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button
              variant="outline"
              size="sm"
              className={cn("h-9 gap-2 font-normal", !range?.from && "text-muted-foreground")}
            >
