...
export function CalendarView({
  ...
  return (
    <div className="space-y-3">
      ...
      {mode === "month" && <MonthView cursor={cursor} appointments={appointments} {...handlers} />}
      {mode === "week" && <WeekView cursor={cursor} appointments={appointments} {...handlers} />}
      {mode === "day" && <DayView cursor={cursor} appointments={appointments} {...handlers} />}
    </div>
  );
}
...
function MonthChip({ appointment, ...handlers }: { appointment: Appointment } & Handlers) {
    ...
    <Popover>
      ...
      <PopoverContent className="w-80 p-0" align="start">
        <div className="border-b bg-muted/30 px-4 py-3">
          <div className="text-sm font-semibold text-foreground">{format(day, "EEEE, MMM d")}</div>
        ...
        <div className="max-h-[320px] space-y-1 overflow-y-auto p-2">
          {items.map((a) => {
            ...
            return (
              <div
                key={a.id}
                className={cn("flex items-center gap-2 rounded-md bg-muted/40 px-3 py-2")}
              >
...
function layoutOverlaps(items: Appointment[]): LaidOut[] {
  ...
  for (const a of sorted) {
    ...
    const used = new Set(cluster.filter((e) => e.endMin > startMin).map((e) => e.column));
    let col = 0;
              ...
              return (
                <div key={d.toISOString()} className="relative border-s border-border/60">
                  ...
                  {layoutOverlaps(items).map(({ appointment: a, column, columnCount }) => {
                    const startD = parseISO(a.start);
                    const startMin = startD.getHours() * 60 + startD.getMinutes();
                    const top = ((startMin - HOURS_START * 60) / 60) * HOUR_HEIGHT_WEEK;
