...
import { InvoiceStatusBadge, ServiceBadge, initialsFromName } from "@/components/billing/badges";
...
export function InvoicesTable({
            ...
            <div className="flex justify-end" onClick={(e) => e.stopPropagation()}>
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48">
                  ...
                  <DropdownMenuItem onClick={() => onSendReminder(inv)} disabled={!canRemind}>
                    <Send className="me-2 h-4 w-4" /> Send Reminder
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => onMarkPaid(inv)} disabled={!canPay}>
  ...
  const hasActiveFilters =
    Boolean(search) || tab !== "All" || service !== "All" || dateRange !== "All time";
  const clearFilters = () => {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select value={service} onValueChange={(v) => setService(v as InvoiceService | "All")}>
      ...
      {isLoading ? (
        <TableSkeleton
          columns={["w-20", "w-40", "w-20", "w-24", "w-24", "w-16", "w-20", "w-20", "w-8"]}
        />
      ) : isError ? (
