...
const DOT_STYLES: Record<DoctorStatus, string> = {
  Available:
    "bg-[color:var(--accent-teal)] shadow-[0_0_0_3px_color-mix(in_oklab,var(--accent-teal)_25%,transparent)]",
...
export function DoctorAvailability() {
  const { data, isLoading, isError, refetch } = useMockQuery(DOCTOR_AVAILABILITY);
  const items = data ?? [];
    ...
    <Card className="card-glass h-full">
      ...
      <CardContent className="p-0">
        {isLoading ? (
          ...
          <ErrorState compact title="Couldn't load roster" onRetry={refetch} />
        ) : items.length === 0 ? (
          <EmptyState
            compact
            icon={Stethoscope}
            title="No doctors on shift"
            description="Nobody is scheduled for this shift."
            action={{ label: "Manage schedule", href: "/doctors" }}
          />
        ) : (
          <>
              ...
              {items.map((d) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-end gap-2">
                    <span
                      className={cn("h-2 w-2 rounded-full", DOT_STYLES[d.status])}
                      aria-hidden
                    />
