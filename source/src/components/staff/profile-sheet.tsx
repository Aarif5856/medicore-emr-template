...
import { formatStaffDate, fullName, initialsOf, type StaffMember } from "@/data/staff";
import { AttendanceBadge, RoleBadge, ShiftBadge, StatusBadge } from "@/components/staff/badges";
      ...
      <SheetHeader className="space-y-4 border-b bg-card p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1 space-y-1">
            ...
            <SheetDescription className="text-xs tabular">{staff.id}</SheetDescription>
          ...
          <TabsContent value="overview" className="space-y-4">
            ...
            <Section title="Employment">
              ...
              {(staff.role === "Doctor" || staff.role === "Nurse") && (
                <InfoRow label="Qualification" value={staff.qualification ?? "-"} full />
              )}
            </Section>
          </TabsContent>
          ...
          <TabsContent value="documents">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {staff.documents.map((d) => (
                <div key={d.id} className="flex items-start gap-3 rounded-lg border bg-card p-3">
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-lg border bg-card">
      ...
      <dl className="grid grid-cols-1 gap-x-6 gap-y-3 p-4 sm:grid-cols-2">{children}</dl>
    </div>
  );
}
